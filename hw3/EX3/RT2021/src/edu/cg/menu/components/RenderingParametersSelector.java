...
public class RenderingParametersSelector extends JPanel {
	...
	private JFormattedTextField planeWidth;
	...
	public void initFields() {
		...
		planeWidth = addAndGetTextField(" Plane Width:", 4.0, panel1);
		...
		x1.setFont(myFont);
		x4 = new Checkbox("x4 ", group, false);
		x4.setFont(myFont);
		x9 = new Checkbox("x9", group, false);
		x9.setFont(myFont);
		panel1.add(x1);
		panel1.add(x4);
		panel1.add(x9);
	...
	public double viewAngle() throws IllegalArgumentException{
		double ans = (Double)planeWidth.getValue();
	...
	public void setPlaneWidth(double width) {
		this.planeWidth.setValue(width);
	}
	...
	public boolean readParameters(Scene scene, MenuWindow menuWindow) {
		...
		if(x1.getState())
			...
			scene.initAntiAliasingFactor(4);
		else
			scene.initAntiAliasingFactor(9);

		scene.initRenderRefractions(refraction.getState())
